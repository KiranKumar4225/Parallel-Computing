...
int success = 0;
int num_thrd = 2;   // num_thrd defines the number of threads used for OpenMP implementation
...
int pass_decrypt(char* plaintext, unsigned char* password, unsigned char* salt, unsigned char* ciphertext, size_t cipher_len)
{
    unsigned char key[16];                      // Function used for decryption and comparing it with plaintext
    unsigned char iv[16];
    ...
    if (success == 1){
        if(checkPlaintext(plaintext, result)==0){
            free(result);
            printf("Password is %s\n", password);           // Once password found it prints and else it returns 1.
            return 0;
        }
    }
...
#pragma omp parallel for schedule(static,1) firstprivate(ciphertext,key,iv) // Parallel for loop concept implemented
                ...
                for( int l=0; l<dict_len; l++){
                    //for( int m=0; m<dict_len; m++){
                            ...
                            *(password+3) = dict[l];
                            ...
                            if(pass_decrypt(plaintext, password, salt, ciphertext, cipher_len) == 0) //function call for decryption and comparision
                            {
                                found=1;
                                time_t end = time(NULL);
                                printf("Time elpased is %ld seconds", (end - begin)); //To print the time once password is found
                ...
                for( int k=0; k<dict_len; k++)
                    for( int l=0; l<dict_len; l++)
                        for( int m=0; m<dict_len; m++){
                                ...
                                unsigned char plainpassword[] = "00000";                // Initialize Password to 000000 to avoid collision
                                unsigned char* password = &plainpassword[0];
                    ...
                    for( int l=0; l<dict_len; l++)
                        for( int m=0; m<dict_len; m++)
                            for( int o=0; o<dict_len; o++){
                                ...
                                unsigned char plainpassword[] = "000000";           // Initialize Password to 000000 to avoid collision
                                unsigned char* password = &plainpassword[0];
